...
describe("inbox model", () => {
  it("stamps an event into an unread, unarchived notification", () => {
  ...
  it("counts unread as not read and not archived", () => {
    const list = [
      entry({ id: "a" }),
      entry({ id: "b", read: true }),
      entry({ id: "c", archived: true }),
      entry({ id: "d", read: true, archived: true }),
    ] as const;
    expect(unreadInboxCount(list)).toBe(1);
    expect(isInboxUnread(list[0])).toBe(true);
    expect(isInboxUnread(list[1])).toBe(false);
    expect(isInboxUnread(list[2])).toBe(false);
  });
  ...
  it("projects the list and archive views with the unread filter", () => {
    const list = [
      entry({ id: "fresh", at: "2026-09-25T10:00:00.000Z" }),
      entry({ id: "seen", at: "2026-09-25T09:00:00.000Z", read: true }),
      entry({ id: "filed", at: "2026-09-25T08:00:00.000Z", archived: true }),
    ];
    expect(visibleInboxNotifications(list, "inbox", false).map((item) => item.id)).toEqual([
      "fresh",
      "seen",
    ]);
    expect(visibleInboxNotifications(list, "inbox", true).map((item) => item.id)).toEqual([
      "fresh",
    ]);
    expect(visibleInboxNotifications(list, "archive", false).map((item) => item.id)).toEqual([
      "filed",
    ]);
    expect(visibleInboxNotifications(list, "archive", true).map((item) => item.id)).toEqual([
      "filed",
    ]);
  });
